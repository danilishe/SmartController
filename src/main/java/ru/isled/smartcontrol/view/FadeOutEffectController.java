...
public class FadeOutEffectController implements Initializable {
    private static List<IntegerProperty> cells;
    private static int sizeX;
    private static int sizeY;
    private static FadeOutEffectController controller;
    @FXML
    Slider endBright;
    ...
    private FadeOutEffectController() {
        loadDialog();
    }

    public static FadeOutEffectController get(List<IntegerProperty> aCells, int x, int y) {
        cells = aCells;
        sizeX = x;
        sizeY = y;

        if (controller == null)
            controller = new FadeOutEffectController();
        return controller;
    }
    ...
    public void apply(double endBr, double startBr, boolean onlyEmpty) {
        ...
        double step = (startBr - endBr) / (sizeY - 1);
    ...
    private void loadDialog() {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("fadeOut.fxml"));
        ...
        window.setTitle("Эффект \"Угасание\"");
